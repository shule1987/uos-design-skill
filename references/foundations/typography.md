...
- 当一个大于 `60px` 的卡片、面板或信息区域以单个数字作为核心信息时，主数字的最终渲染尺寸不得低于 `24px`；实现时优先映射到满足该阈值的语义字号层级，而不是在业务组件里随意发明新的数字字号。
- 如果同一个卡片或区域内并列展示多个同级数字信息，例如统计矩阵、对比组或多指标摘要，则上述单数字最小字号约束不强制适用。
