...
## Dialog
- 用途：作为桌面应用中的通用模态对话框 fallback，用于确认、提醒、轻量表单和流程分支选择。
- 结构：通常拆分为头部、内容区和底部操作区三层；头部放标题与必要关闭入口，内容区承载说明文本或表单，底部操作区放确认与取消按钮。
- 视觉：标题使用 `Theme.textStrong` 和标题级语义字号，正文使用 `Theme.textPrimary` 或 `Theme.textSecondary`，容器背景使用 `Theme.panelBg`，遮罩使用 `Theme.scrim`。
