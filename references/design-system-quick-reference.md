...
- `Theme.cardBg`：卡片背景，默认相对主窗口背景中性提亮约 `20%`。
...
- 卡片本身和卡片内部都应服从明确栅格。先定义外部列数与跨度，再定义内部媒体区、文本区、指标区和操作区的对齐关系。
- 同一视觉行里相邻卡片若高度差不足约 `40%`，无论处于双列带还是多列卡片墙，优先补齐到该行较高卡片并保持上下边界对齐；跨列卡片默认可保留独立高度。
- 自动生成的结构缩略图应采用弱对比度处理，视觉上退居辅助层，不得压过卡片内文字、数值和交互控件。
...
- 卡片内部元素也应随断点重排，保持稳定 gutter、基线和阅读顺序，而不是只让外壳响应式。
