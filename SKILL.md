...
- When two or more mutually exclusive filter, mode, or state buttons appear together and the local DTK export map includes `ButtonBox`, `ButtonGroup`, or `ControlGroup`, default to a grouped-button path such as `D.ButtonBox` or another validated local DTK group control. Keep the whole option set on one row and ensure each button remains fully visible. Do not render those mutually exclusive options as unrelated standalone buttons, and do not let the set wrap into multiple rows unless an explicit `uos-design: allow-wrapped-mutually-exclusive-group` waiver explains the constraint.
...
- Two or more mutually exclusive filter, mode, or state buttons are rendered as unrelated standalone buttons, or the set wraps into multiple rows despite local DTK group-control availability, and no explicit `uos-design: allow-wrapped-mutually-exclusive-group` waiver explains the exact platform or width constraint.
...
- Whether two or more mutually exclusive filter, mode, or state buttons use a local DTK group-button path such as `D.ButtonBox`, `D.ButtonGroup`, or `D.ControlGroup`, and whether that whole option set stays on one fully visible row.
