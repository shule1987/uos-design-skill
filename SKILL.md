...
3. Prefer DTK controls whenever the local export map says they exist.
4. Use narrow fallbacks only when you can name the exact missing DTK capability, version gap, or platform constraint.
5. Run `scripts/audit_uos_qml.sh <repo-root>` before substantial QML edits and again before finishing. Treat findings as blocking unless fixed or covered by a narrow waiver comment.
6. When the user asks for a review, load `references/review-checklist.md`, then only the smallest matching file under `references/review/`, and load `references/review/close-out.md` before finalizing a substantial review close-out.
...
- Every symbolic app-side button in the unified header, including `leftContent` affordances and page-switch buttons, must explicitly use a `16x16` icon box unless a narrow waiver explains the exception.
